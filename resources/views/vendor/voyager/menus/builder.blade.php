...
@section('page_header')
    <h1 class="page-title">
        <i class="voyager-list"></i>@lang('generic.menu_builder') ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="voyager-plus"></i>@lang('menu_builder.new_menu_item')</div>
    </h1>
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">@lang('menu_builder.drag_drop_info')</p>
                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="@lang('generic.close')"><span
                            aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="voyager-trash"></i>@lang('menu_builder.delete_item_question')</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('voyager.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                        ...
                        <input type="submit" class="btn btn-danger pull-right delete-confirm"
                               value="('menu_builder.delete_item_confirm')">
                    </form>
                    <button type="button" class="btn btn-default pull-right" data-dismiss="modal">@lang('generic.cancel')</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div><!-- /.modal -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="@lang('generic.close') "><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="voyager-plus"></i>@lang('menu_builder.create_new_item')</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="voyager-edit"></i>@lang('menu_builder.edit_item')</h4>
                </div>
                <form action="" id="m_form" method="POST"
                    ...
                    <div class="modal-body">
                        <div>
                            @include('voyager::multilingual.language-selector')
                            <label for="name">@lang('menu_builder.item_title')</label>
                            @include('voyager::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                            <input type="text" class="form-control" id="m_title" name="title" placeholder="@lang('generic.title')"><br>
                        </div>
                        <label for="type">@lang('menu_builder.link_type')</label>
                        <select id="m_link_type" class="form-control" name="type">
                            <option value="url" selected="selected">@lang('menu_builder.static_url')</option>
                            <option value="route">@lang('menu_builder.dynamic_route')</option>
                        </select><br>
                        <div id="m_url_type">
                            <label for="url">@lang('menu_builder.url')</label>
                            <input type="text" class="form-control" id="m_url" name="url" placeholder="@lang('generic.url')"><br>
                        </div>
                        <div id="m_route_type">
                            <label for="route">@lang('menu_builder.item_route')</label>
                            <input type="text" class="form-control" id="m_route" name="route" placeholder="@lang('generic.route')"><br>
                            <label for="parameters">@lang('menu_builder.route_parameter')</label>
                        ...
                        <label for="icon_class">@lang('menu_builder.icon_class')<a
                                    href="{{ route('voyager.compass.index') }}#fonts"
                                    target="_blank">@lang('menu_builder.icon_class2')</label>
                        <input type="text" class="form-control" id="m_icon_class" name="icon_class"
                               placeholder="@lang('menu_builder.icon_class_ph')"><br>
                        <label for="color">@lang('menu_builder.color')</label>
                        <input type="color" class="form-control" id="m_color" name="color"
                               placeholder="@lang('menu_builder.color_ph')"><br>
                        <label for="target">@lang('menu_builder.open_in')</label>
                        <select id="m_target" class="form-control" name="target">
                            <option value="_self" selected="selected">@lang('menu_builder.open_same')</option>
                            <option value="_blank">@lang('menu_builder.open_new')</option>
                        </select>
                        <input type="hidden" name="menu_id" value="{{ $menu->id }}">
                        <input type="hidden" name="id" id="m_id" value="">
                    </div>
                    <div class="modal-footer">
                        <input type="submit" class="btn btn-success pull-right delete-confirm__" value="@lang('generic.update')">
                        <button type="button" class="btn btn-default pull-right" data-dismiss="modal">@lang('generic.cancel')</button>
                    </div>
                </form>
            </div><!-- /.modal-content -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.add_item').click(function() {
                ...
                $m_form.find("input[type=submit]").val('@lang('generic.add')');
            ...
            $('.item_actions').on('click', '.edit', function (e) {
                $m_form.find("input[type=submit]").val('@lang('generic.update')');
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('voyager.menus.order_item',['menu' => $menu->id]) }}', {
                    ...
                    toastr.success("@lang('menu_builder.updated_order')");
                });
            });
        });
    </script>
